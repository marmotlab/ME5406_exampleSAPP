...
class ACNet:
    def __init__(self, scope, a_size, trainer, TRAINING, GRID_SIZE):
        with tf.variable_scope(str(scope)+'/qvalues'):
            ...
            self.policy, self.value, self.state_out, self.state_in, self.state_init, self.valids = self._build_net(self.myinput,self.goal_pos,RNN_SIZE,a_size)
    ...
    def _build_net(self,inputs,goal_pos,RNN_SIZE,a_size):
        ...
        goal_layer= layers.fully_connected(inputs=goal_pos, num_outputs=GOAL_REPR_SIZE) # 1x12
        hidden_input=tf.concat([flat,goal_layer],1) # # 1x116 + # 1x12 = # 1x128
        h1 = layers.fully_connected(inputs=hidden_input,  num_outputs=RNN_SIZE) # 1x128
        h2 = layers.fully_connected(inputs=h1,  num_outputs=RNN_SIZE, activation_fn=None) # 1x128
        ...
        c_init = np.zeros((1, lstm_cell.state_size.c), np.float32) # 1x128 full of zeros
        h_init = np.zeros((1, lstm_cell.state_size.h), np.float32) # 1x128 full of zeros
        state_init = [c_init, h_init]

        c_in = tf.placeholder(tf.float32, [1, lstm_cell.state_size.c]) # 1x128, current state/memory
        h_in = tf.placeholder(tf.float32, [1, lstm_cell.state_size.h]) # 1x128, current state/memory
        state_in = (c_in, h_in)
        ...
        self.rnn_out = tf.reshape(lstm_outputs, [-1, RNN_SIZE]) # output of the LSTM (from h3 + LSTM state)
        ...
        policy       = tf.nn.softmax(policy_layer) # 1x5 of action activation (normalized probability vector)
